...
const NavLink = ({ href, children, className = '', onClick }: NavLinkProps) => {
  ...
  return (
    <Link
      href={href}
      className={`relative group px-3 cursor-pointer py-2 transition-all duration-300 ease-in-out font-bold text-gray-700 hover:text-purple-600 ${className}`.trim()}
      onClick={onClick}
    >
      {children}
